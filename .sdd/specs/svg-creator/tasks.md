...
- [ ] 2.5 ShapeLayerBuilder（領域抽出）を完成させる
  ...
  - [ ] 2.5.2 穴領域（Holes）を検出し `ShapeLayer.Holes` を構築する
    - [ ] 2.5.2.1 輪郭階層とポリゴン包含条件を整理し検出フローを明文化する
    - [ ] 2.5.2.2 OpenCvSharp の階層出力から穴候補を抽出するヘルパーを実装する
    - [ ] 2.5.2.3 抽出結果を `ShapeLayer.Holes` へマッピングし外周との整合を確認する
  - [ ] 2.5.3 微小成分の除去ロジック（`noisyThreshold`）を導入する
    - [ ] 2.5.3.1 design.md のノイズ基準を読み合わせし閾値ルールを決定する
    - [ ] 2.5.3.2 連結成分ごとの面積・周長フィルタを実装する
    - [ ] 2.5.3.3 フィルタ結果をレイヤ構築フローへ反映し副作用を点検する
  - [ ] 2.5.4 ノイズ層を `NoisyLayer[]` として分類・返却する
    - [ ] 2.5.4.1 `NoisyLayer` に保持すべき情報と初期化手順を決める
    - [ ] 2.5.4.2 除外済み成分から `NoisyLayer` を生成する処理を実装する
    - [ ] 2.5.4.3 呼び出し側 API にノイズ層返却を組み込みハンドリングを確認する
  - [ ] 2.5.5 境界/穴/ノイズケースを網羅する単体テストを追加する
    - [ ] 2.5.5.1 境界と穴を含むテストフィクスチャを準備する
    - [ ] 2.5.5.2 `noisyThreshold` の境界値ケースを再現するテストを作成する
    - [ ] 2.5.5.3 ノイズ層返却と穴構築の期待値を検証するテストを追加する
